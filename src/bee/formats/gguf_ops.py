...
def _read_string(f: IO[bytes]) -> str:
    length = _read_u64(f)
...
def _read_scalar(f: IO[bytes], vtype: int) -> Any:
    if vtype == _STRING:
...
def _skip_array_elements(f: IO[bytes], elem_type: int, count: int) -> None:
    ...
    if elem_type == _STRING:
        ...
        for _ in range(count):
            ...
            _skip_array_elements(f, sub_elem_type, sub_count)
...
def _read_kv_value(f: IO[bytes], vtype: int) -> Any:
    if vtype == _ARRAY:
        ...
        _skip_array_elements(f, elem_type, count)
...
def analyze_gguf(path: Path) -> GgufAnalysis | None:
    ...
    try:
        ...
        with path.open("rb") as f:
            ...
            for _ in range(kv_count):
                key = _read_string(f)
                vtype = _read_u32(f)
                metadata[key] = _read_kv_value(f, vtype)
    ...
    except (OSError, _GgufParseError, struct.error, UnicodeDecodeError, MemoryError):
        return None
